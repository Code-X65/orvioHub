...
export function normalizeSubdomain(sub) {
    const s = sub.toLowerCase();
    if (s === "account" || s === "accounts")
        return "accounts";
    if (s === "app" || s === "launcher" || s === "home")
        return "home";
    if (s === "pos" || s === "inventory")
        return "inventory";
    if (s === "tasks" || s === "taskmanagement")
...
export function resolveHost(rawHost, pathname = "") {
    ...
    if (normalizedSub === "accounts")
        return { environment, application: "accounts", hostname };
    if (normalizedSub === "home")
        return { environment, application: "home", hostname };
    if (normalizedSub === "inventory")
        return { environment, application: "inventory", hostname };
    if (normalizedSub === "billing")
        return { environment, application: "billing", hostname };
    if (normalizedSub === "taskmanagement")
        return { environment, application: "taskmanagement", hostname };
    const match = Object.values(applications).find((a) => a.enabled && a.subdomain !== "" && (a.subdomain === normalizedSub || a.subdomain === sub));
    if (!match)
        throw new UnknownHostError(hostname);

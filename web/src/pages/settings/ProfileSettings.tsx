...
export const ProfileSettings: React.FC = () => {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.patch<{ user: any }>('/auth/profile', {
        name: name.trim(),
        timezone,
      });
      updateUser({ name: res.user.name, timezone: res.user.timezone });
              ...
              <form onSubmit={handleSaveProfile} className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="mt-1.5 w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                  >
                    {TIMEZONES.map((tz) => (

        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {user?.twoFactorEnabled ? (
              <span className="text-[11px] font-medium text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" />
                Enabled & Active
              </span>
            ) : (
              <span className="text-[11px] font-medium text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-full">
                Not Enabled
              </span>
            )}
          </div>
          ...
          {!user?.twoFactorEnabled ? (
            <Button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs px-4 flex items-center gap-1.5"
            >
            ...
            <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between">
              ...
              {identities.some((i) => i.provider === 'google') ? (
                <Button
                  ...
                  className="border-white/10 bg-transparent text-rose-400 hover:bg-rose-950/30 text-xs"
                >
                  Disconnect
                </Button>
              ) : (
            ...
            {!backupCodes ? (
              <form onSubmit={handleVerify2fa} className="space-y-4">
                <div className="p-3 bg-black/60 rounded-xl border border-white/10 space-y-2">
                  ...
                  <div className="p-2 bg-white/5 rounded font-mono text-center text-xs text-white tracking-widest break-all">
                    {twoFactorSecret}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-3 bg-black/60 rounded-xl border border-white/10 font-mono text-xs text-slate-200 grid grid-cols-2 gap-2">
                  {backupCodes.map((code, idx) => (
                    <div key={idx} className="p-1.5 bg-white/5 rounded text-center">
                      {code}
                    </div>
                  ))}
                </div>
                ...
                <Button
                  type="button"
                  onClick={() => setIs2faModalOpen(false)}
                  className="w-full bg-[#714b67] hover:bg-[#88597c] text-white text-xs"
                >

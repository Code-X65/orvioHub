...
export const NIGERIAN_STATES = [
  ...
  },
  {
    ...
    lgas: [
      ...
      "Ido Osi", "Ijero", "Ikole", "Ilejemeje", "Irepodun/Ifelodun", "Ise/Orun", "Moba", "Oye"
    ]
  },
  {
    name: "Enugu",
...
// Query: Get all Nigerian states sorted alphabetically
export const getStates = query({
  ...
  handler: async (ctx) => {
    ...
    // Fallback to in-memory definition if database not yet seeded
    if (!states || states.length === 0) {
      return NIGERIAN_STATES.map((s) => ({
        _id: `fallback-${s.code}` as any,
        type: "state" as const,
        name: s.name,
        code: s.code,
        stateCode: s.code,
...
// Query: Get LGAs by stateCode
export const getLgas = query({
  args: {
    stateCode: v.string(),
  },
  handler: async (ctx, args) => {
    const normalizedCode = args.stateCode.trim().toUpperCase();
    const lgas = await ctx.db
      .query("locations")
      .withIndex("by_type_state", (q) =>
        q.eq("type", "lga").eq("stateCode", normalizedCode)
      )
      .collect();
    ...
    if (!lgas || lgas.length === 0) {
      const stateMatch = NIGERIAN_STATES.find(
        (s) => s.code.toUpperCase() === normalizedCode || s.name.toLowerCase() === args.stateCode.toLowerCase()
      );
      if (!stateMatch) return [];
      return stateMatch.lgas.map((lga, idx) => ({
        _id: `fallback-${normalizedCode}-${idx}` as any,
        type: "lga" as const,
        name: lga,
...
// Mutation: Seed Nigerian locations database (36 states + FCT + 774 LGAs)
export const seedNigerianLocations = mutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    if (existingStates.length > 0 && !args.force) {
      const allStates = await ctx.db
        .query("locations")
        .withIndex("by_type", (q) => q.eq("type", "state"))
        .collect();
      const allLgas = await ctx.db
        .query("locations")
        .withIndex("by_type", (q) => q.eq("type", "lga"))
        .collect();
      return {
        success: true,
        message: "Locations database already seeded.",
        statesCount: allStates.length,
        lgasCount: allLgas.length,
      };
    }
    ...
    for (const state of NIGERIAN_STATES) {
      ...
      const existingState = await ctx.db
        .query("locations")
        .withIndex("by_type_state", (q) =>
          q.eq("type", "state").eq("stateCode", state.code)
        )
        .first();

      let stateId = existingState?._id;

      if (!stateId) {
        stateId = await ctx.db.insert("locations", {
          type: "state",
          name: state.name,
          code: state.code,
      ...
      // Insert all LGAs for this state
      for (const lgaName of state.lgas) {
        const existingLga = await ctx.db
          .query("locations")
          .withIndex("by_type_state", (q) =>
            q.eq("type", "lga").eq("stateCode", state.code)
          )
          .filter((q) => q.eq(q.field("name"), lgaName))
          .first();
    ...
    return {
      ...
      totalLgas: NIGERIAN_STATES.reduce((acc, s) => acc + s.lgas.length, 0),
    };
  },
});

...
export const authRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = await dataService.rotateSession(parsed.data.refreshToken, userAgent, ipAddress);
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      await dataService.logoutUser(request.user.id, body.refreshToken);
      await dataService.logAudit({
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!env.GOOGLE_CLIENT_ID || !env.GOOGLE_CLIENT_SECRET) {
        ...
        return reply.redirect(`${env.APP_URL}/auth/callback?error=${ERROR_CODES.OAUTH_NOT_CONFIGURED}`);
      }
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (error) {
        return reply.redirect(`${env.APP_URL}/auth/callback?error=${ERROR_CODES.OAUTH_ACCESS_DENIED}`);
      }

      if (!state) {
        return reply.redirect(`${env.APP_URL}/auth/callback?error=${ERROR_CODES.OAUTH_STATE_INVALID}`);
      }
      ...
      try {
        ...
        if (!code) {
          return reply.redirect(`${env.APP_URL}/auth/callback?error=${ERROR_CODES.OAUTH_CODE_INVALID}`);
        }
        ...
        return reply.redirect(`${env.APP_URL}/auth/callback?token=${jwtToken}&refreshToken=${session.refreshToken}`);
      } catch (err: any) {
        ...
        return reply.redirect(`${env.APP_URL}/auth/callback?error=${errorCode}`);
      }
    }
  );

...
import { getLauncherUrl, getHomeUrl } from '@orviohub/shared';
...
export const VerifyEmail: React.FC = () => {
  ...
  // Path segments survive Brevo/Resend click-tracking rewrites; query params can be stripped.
  ...
  useEffect(() => {
    ...
    if (user?.emailVerified) {
      ...
      const target = new URL(`${getLauncherUrl(env)}/onboarding`);
  ...
  }, [user?.emailVerified, env, searchParams, navigate]);
  ...
  const executeVerification = async (verifyToken: string) => {
    ...
    try {
      ...
      // Determine appropriate onboarding target step
      const step = response.onboarding?.currentStep;
      let targetBase = getLauncherUrl(env);
      let targetPath = '/onboarding';
      if (response.onboarding?.status === 'COMPLETED') {
        targetBase = getHomeUrl(env);
        targetPath = '';
      } else if (step === 'ORGANIZATION_CONFIGURED' || step === 'MODULE_SELECTION') {
        targetPath = '/onboarding/modules';
      } else if (step === 'WORKSPACE_INITIALIZATION') {
        targetPath = '/onboarding/workspace';
      } else if (step === 'WORKSPACE_READY' || step === 'TEAM_INVITATION') {
        targetPath = '/onboarding/team';
      }

      const target = new URL(`${targetBase}${targetPath}`);
      if (response.token) target.searchParams.set('auth_token', response.token);
      if (response.refreshToken) target.searchParams.set('refresh_token', response.refreshToken);
    ...
    return (
      <AuthLayout>
        <div className="flex flex-col items-center justify-center text-center space-y-4 py-6 animate-in fade-in duration-200">
          ...
          <p className="text-xs text-slate-300">Your account is active. Taking you to Orviohub...</p>
          <Button
            onClick={() => {
              if (redirectUrl) {
                window.location.href = redirectUrl;
              } else {
                window.location.href = `${getLauncherUrl(env)}/onboarding`;
              }
            }}
            className="w-full h-11 bg-[#714b67] hover:bg-[#86597a] text-white rounded-xs text-xs font-semibold cursor-pointer"
          >
            <span>Proceed to Get Started</span>
      ...
      <div className="flex flex-col items-center justify-center text-center space-y-5 py-4 animate-in fade-in duration-200">
        ...
        <div className="w-full space-y-3 pt-2">
          ...
          <div className="pt-2 flex items-center justify-between text-xs text-slate-400">
            ...
            <Link to="/app" className="text-[#c79dbd] hover:text-white font-semibold transition-colors">
              Continue to App Launcher →
            </Link>
          </div>
        </div>
      </div>
